...
class Spacebrew {
  ...
  public void connect( String _hostname, Integer _port, String _name, String _description ) {
    ...
    try {
      ...
      //wsClient = new WsClient( sb, ("ws://" + hostname + ":" + Integer.toString(_port)) );      
      wsClient.connect();
      updatePubSub();
    }
    catch (Exception e) {
  ...
  private void updatePubSub() {
    ...
    for (int i=0, len=publishes.size(); i<len; i++) {
      ...
      //publishers.put(pub);      
      publishers.setJSONObject(i, pub);
    }
    ...
    for (int i=0; i<subscribes.size(); i++) {
      ...
      subscribers.setJSONObject(i,subs);
    }
  ...
  public void onMessage( String message ) {
    ...
    JSONObject m = parseJSONObject(message);
